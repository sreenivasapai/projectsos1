...
public class MainActivity extends AppCompatActivity implements NavigationView.OnNavigationItemSelectedListener {
    ...
    private void makeCallToContact(int index) {
        if (index >= phoneNumbers.length) {
            // All calls attempted
            Toast.makeText(this, "All SOS calls completed", Toast.LENGTH_SHORT).show();
            return;
        }
        ...
        if (ContextCompat.checkSelfPermission(this, Manifest.permission.CALL_PHONE) == PackageManager.PERMISSION_GRANTED) {
            ...
            // Delay before attempting the next call (e.g., 30 seconds)
            new CountDownTimer(3000, 100) { // Adjust timeout duration as needed
                public void onTick(long millisUntilFinished) {
    ...
    public boolean onNavigationItemSelected(@NonNull MenuItem item) {
        ...
        if (id == R.id.add_contact) {
            ...
            Intent intent = new Intent(MainActivity.this, SettingsActivity.class); // Correct Intent for Settings
            startActivity(intent);
